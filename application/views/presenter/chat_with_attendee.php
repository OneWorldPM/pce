...
<style>
    ...
    .presenter-to-user-text-presenter{
        color: white;
    ...
    .user-to-presenter-text-presenter{
        color: white;
...
<script>
    ...
    var socket_session_name = "<?=getAppName('_presenter-to-attendee-chat')?>";
    ...
    $(document).ready(function () {
        ...
        $('.session-name-item').on('click', function () {
            ...
            $.post(base_url+"presenter/sessions/getAllUsersList",
        ...
        $('.users-list').on('click', '.users-list-item', function () {
            ...
            $.post(base_url+"presenter/sessions/getAllPresenterToAttendeeChat",
                ...
                $.get(base_url+"presenter/sessions/markAllAsRead/"+session_id+'/'+user_id, function( data ) {
                    if (data == 1)
                    {
                ...
                {
                    if (chat.from_id == 'presenter'){
                        $('.chat-body').append('' +
                            '<span class="presenter-to-user-text-presenter">'+chat.chat_text+'</span>');
                    }else{
                        $('.chat-body').append('' +
                            '<span class="user-to-presenter-text-presenter"><strong style="margin-right: 10px">'+user_name+'</strong>'+chat.chat_text+'</span>');
                    }
                });
        ...
        $('.send-text-btn').on('click', function () {
            ...
            $.post(base_url+"presenter/sessions/savePresenterToAttendeeChat",

                {
                    ...
                    if (data == 1)
                    {
                        socket.emit('new-attendee-to-prensenter-chat', {"socket_session_name":socket_session_name, "session_id":sessionId, "from_id":"presenter", "to_id":userId, "chat_text":message});

                        $('.chat-body').append('' +
                            '<span class="presenter-to-user-text-presenter">'+message+'</span>');
        ...
        socket.on('new-attendee-to-presenter-chat-notification', function (data) {
            if (data.socket_session_name == socket_session_name)
            {
                if (data.from_id != 'presenter')
                {
                    if ($('.chat-body').attr('user-id') == data.from_id)
                    {
                        ...
                        $('.chat-body').append('' +
                            '<span class="user-to-prenseter-text-presenter"><strong style="margin-right: 10px">'+user_name+'</strong>'+data.chat_text+'</span>');
